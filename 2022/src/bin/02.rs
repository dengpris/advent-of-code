...
fn calculate_score(player_move: Move, opponent_move: Move) -> u64 {
        if player_move == opponent_move { return 3; } // Draw
...
pub fn part_one(input: &str) -> Option<u64> {
    let res = input.lines().map(|line| {
            let opponent_move = Move::get_move(line.chars().next().unwrap_or_default()).unwrap_or(Move::Rock);
            let player_move = Move::get_move(line.chars().nth(2).unwrap_or_default()).unwrap_or(Move::Rock);
...
pub fn part_two(input: &str) -> Option<u64> {
    let res = input.lines().map(|line| {
            ...
            let player_move = opponent_move.get_move_from_outcome(outcome).unwrap_or(Move::Rock);
